...
bool reportPllStatus(const char* context) {
  ...
  if (pll_unlocked) {
    Serial.println("PLL remains unlocked after BIOZ impedance sweep is active; check the MAX30001G external clock/oscillator.");
  }

  return !pll_unlocked;
}

void printSweepConfig() {
  Serial.println();
  Serial.println("BIOZ internal impedance sweep setup:");
  Serial.print("speed=");
...
bool collectSweepPoint(float& mean_ohm, float& min_ohm, float& max_ohm, uint16_t& samples) {
  mean_ohm = 0.0f;
  min_ohm = 0.0f;
  max_ohm = 0.0f;
  samples = 0U;
...
void printSweepPoint(uint32_t requested_frequency_hz, float phase_deg) {
  ...
  float max_ohm = 0.0f;
  uint16_t samples = 0U;
  const bool complete = collectSweepPoint(mean_ohm, min_ohm, max_ohm, samples);
...
void runSweep() {
  ...
  for (uint8_t f = 0U; f < (sizeof(SWEEP_FREQUENCIES_HZ) / sizeof(SWEEP_FREQUENCIES_HZ[0])); ++f) {
    for (uint8_t p = 0U; p < (sizeof(SWEEP_PHASES_DEG) / sizeof(SWEEP_PHASES_DEG[0])); ++p) {
      printSweepPoint(SWEEP_FREQUENCIES_HZ[f], SWEEP_PHASES_DEG[p]);
    }
  }

  afe.stop();
  Serial.println("BIOZ internal impedance sweep complete.");
}
...
void setup() {
  ...
  Serial.println("MAX30001G BIOZ internal impedance sweep started.");
